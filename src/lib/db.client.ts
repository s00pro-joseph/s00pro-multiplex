...
// ---- 常量 ----
const PLAY_RECORDS_KEY = 's00pro-multiplex_play_records';
const FAVORITES_KEY = 's00pro-multiplexultiplex_favorites';
const REMINDERS_KEY = 's00pro-multiplexultiplex_reminders'; // 提醒存储键
const SEARCH_HISTORY_KEY = 's00pro-multiplexultiplex_search_history';
const USER_STATS_KEY = 's00pro-multiplexultiplex_user_stats'; // 添加用户统计数据存储键

// 缓存相关常量
const CACHE_PREFIX = 's00pro-multiplexultiplex_cache_';
...
class HybridCacheManager {
  ...
  private clearAllCache(): void {
    ...
    keys.forEach((key) => {
      if (key.startsWith('s00pro-multiplexultiplex_cache_')) {
...
export async function getSkipConfig(
  ...
  try {
    ...
    if (STORAGE_TYPE === 'localstorage') {
      // localStorage 模式
      const raw = localStorage.getItem('s00pro-multiplexultiplex_skip_configs');
      if (!raw) return null;
  ...
  try {
    ...
    if (STORAGE_TYPE === 'localstorage') {
      ...
      localStorage.setItem(
        's00pro-multiplexultiplex_skip_configs',
