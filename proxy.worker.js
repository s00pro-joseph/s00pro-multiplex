...
const JSON_SOURCES = {
  // 预留：填入自有 LunaTV JSON 配置源后，?format=1&source=xxx 方可用
  // 例：'full': 'https://raw.githubusercontent.com/<you>/<repo>/main/LunaTV-config.json'
}
...
async function handleFormatRequest(formatParam, sourceParam, prefixParam, defaultPrefix) {
  try {
    ...
    if (!selectedSource) {
      return errorResponse('JSON source not configured', { source: sourceParam }, 501)
    }
